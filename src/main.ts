...
import {replaceAsync, cleanContent, getAttachmentFolderFullPath} from "./utils";
import {
    ISettings,
    DEFAULT_SETTINGS,
...
export default class LocalImagesPlugin extends Plugin {
    ...
    private async proccessPage(file: TFile, silent = false) {
        ...
        await this.ensureFolderExists(getAttachmentFolderFullPath(file.parent.path, file.basename, this.settings.mediaRootDirectory));
        ...
        const fixedContent = await replaceAsync(
            cleanedContent,
            EXTERNAL_MEDIA_LINK_PATTERN,
            imageTagProcessor(this.app, this.settings.mediaRootDirectory, file)
        );
